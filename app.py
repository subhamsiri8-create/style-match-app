...
def get_exact_fabric_color(image):
    try:
        ...
        # Precision Crop: Focus on center mass to ignore backgrounds
        cy, cx = h // 2, w // 2
        ...
        # Score clusters for vibrancy (ignores shadows and whites)
        best_rgb = [128, 128, 128] # Default Grey fallback
        max_score = -1
        for color in clt.cluster_centers_:
            r, g, b = color[::-1] / 255.0 # BGR to RGB
            h_val, l_val, s_val = colorsys.rgb_to_hls(r, g, b)
            # Prioritize saturation while avoiding black/white extremes
        ...
        return [128, 128, 128] # Absolute fallback to prevent crash
...
garment_choice = st.radio("What are you matching?", ["Kurta (Ethnic)", "Saree (Ethnic)", "Shirt (Western)"], horizontal=True)
...
if uploaded_file:
    ...
    with st.spinner("Extracting exact fabric hue..."):
        ...
        # Safe CSS generation to prevent StreamlitAPIException
    ...
    with col2:
        ...
        st.divider()
        # Fallback picker with safe hex value
...
# Professional Footer with Developer Link
st.markdown("---")
